 ...
 */
class Salore_Mssql_Adapter_Abstract extends Zend_Db_Adapter_Pdo_Mssql implements Varien_Db_Adapter_Interface {
     ...
     * If FK with same name exist - it will be deleted
     *
     ...
     * @return mixed
     */
    public function addConstraint($fkName, $tableName, $columnName, $refTableName, $refColumnName, $onDelete = Varien_Db_Adapter_Interface::FK_ACTION_CASCADE, $onUpdate = Varien_Db_Adapter_Interface::FK_ACTION_CASCADE, $purge = false) {
        return $this->addForeignKey ( $fkName, $tableName, $columnName, $refTableName, $refColumnName, $onDelete, $onUpdate, $purge );
    }
    ...
    public function saveDdlCache($tableCacheKey, $ddlType, $data) {
        ...
        if ($this->_cacheAdapter instanceof Zend_Cache_Core) {
            $cacheId = $this->_getCacheId ( $tableCacheKey, $ddlType );
            $data = serialize ( $data );
            $this->_cacheAdapter->save ( $data, $cacheId, array (
                    static::DDL_CACHE_TAG 
            ) );
        }
    ...
    public function resetDdlCache($tableName = null, $schemaName = null) {
        ...
        if ($tableName === null) {
            ...
            if ($this->_cacheAdapter instanceof Zend_Cache_Core) {
                $this->_cacheAdapter->clean ( Zend_Cache::CLEANING_MODE_MATCHING_TAG, array (
                        static::DDL_CACHE_TAG 
                ) );
            }
        } else {
    ...
    public function createTableByDdl($tableName, $newTableName) {
        ...
        foreach ( $indexes as $indexData ) {
             ...
             */
            if (($indexData ['KEY_NAME'] == 'PRIMARY') || ($indexData ['INDEX_TYPE'] == Varien_Db_Adapter_Interface::INDEX_TYPE_PRIMARY)) {
                continue;
            }
        ...
        foreach ( $foreignKeys as $keyData ) {
            ...
            $table->addForeignKey ( $fkName, $keyData ['COLUMN_NAME'], $keyData ['REF_TABLE_NAME'], $keyData ['REF_COLUMN_NAME'], $onDelete, $onUpdate );
        }
    ...
    public function insertIgnore($table, array $bind) {
        ...
        // build the statement
        $sql = "INSERT IGNORE INTO " . $this->quoteIdentifier ( $table, true ) . ' (' . implode ( ', ', $cols ) . ') ' . 'VALUES (' . implode ( ', ', $vals ) . ')';
    ...
    protected function _getColumnDefinition($options, $ddlType = null) {
        ...
        /*
         * For cases when tables created from createTableByDdl() where default value can be quoted already. We need to avoid "double-quoting" here
         */
     ...
     */
    public function addIndex($tableName, $indexName, $fields, $indexType = Varien_Db_Adapter_Interface::INDEX_TYPE_INDEX, $schemaName = null) {
        ...
        while ( $cycle === true ) {
            try {
                ...
                if (in_array ( strtolower ( $indexType ), array (
                        'primary',
                        'unique' 
                ) )) {
                    $match = array ();
     ...
     * @return Varien_Db_Adapter_Pdo_Mysql
     */
    public function addForeignKey($fkName, $tableName, $columnName, $refTableName, $refColumnName, $onDelete = Varien_Db_Adapter_Interface::FK_ACTION_CASCADE, $onUpdate = Varien_Db_Adapter_Interface::FK_ACTION_CASCADE, $purge = false, $schemaName = null, $refSchemaName = null) {
     ...
     *            value to use if value doesn't conform to any cases
     *            
     * @return Zend_Db_Expr
     */
    ...
    public function selectsByRange($rangeField, Varien_Db_Select $select, $stepCount = 100) {
        ...
        $selectRange = $this->select ()->from ( $tableName, array (
    ...
    public function updateFromSelect(Varien_Db_Select $select, $table) {
        ...
        foreach ( $select->getPart ( Zend_Db_Select::COLUMNS ) as $columnEntry ) {
            ...
            if (! $column instanceof Zend_Db_Expr && ! empty ( $correlationName )) {
                $column = $this->quoteIdentifier ( array (
                        $correlationName,
                        $column 
                ) );
            }
            $columns [] = sprintf ( '%s = %s', $this->quoteIdentifier ( array (
                    $tableAlias,
                    $alias 
            ) ), $column );
        }
    ...
    public function getTablesChecksum($tableNames, $schemaName = null) {
        $result = array ();
        $tableNames = is_array ( $tableNames ) ? $tableNames : array (
                $tableNames 
        );
     ...
     * Deprecated and left for compatibility only.
    ...
    public function dropTrigger($triggerName) {
        $query = sprintf ( 'DROP TRIGGER IF EXISTS %s', $this->_getTableName ( $triggerName ) );
        $this->query ( $query );
        return $this;
    }
    ...
    public function createTableFromSelect($tableName, Zend_Db_Select $select, $temporary = false) {
        $query = sprintf ( 'CREATE' . ($temporary ? ' TEMPORARY' : '') . ' TABLE `%s` AS (%s)', $this->_getTableName ( $tableName ), ( string ) $select );
        $this->query ( $query );
    }
}
