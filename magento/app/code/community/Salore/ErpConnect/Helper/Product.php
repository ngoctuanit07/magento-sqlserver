...
class Salore_ErpConnect_Helper_Product extends Mage_Core_Helper_Abstract {
    ...
    public function createProduct($data) {
        ...
        try {
            ...
            setWeight ( 4.0000 )->setStatus ( 1 )->            // product status (1 - enabled, 2 - disabled)
            setTaxClassId ( 4 )->            // tax class (0 - none, 1 - default, 2 - taxable, 4 - shipping)
            setVisibility ( Mage_Catalog_Model_Product_Visibility::VISIBILITY_BOTH )->            // catalog and search visibility
            setNewsToDate ( '06/30/2014' )->            // product set as new to
            setCountryOfManufacture ( 'AF' )->            // country of manufacture (2-letter country code)
            setPrice ( $data ['Price'] )->            // price in form 11.22
            setDescription ( $data ['ShortDesc'] )->setShortDescription ( $data ['ShortDesc'] )->setMediaGallery ( array (
                    'images' => array (),
                    'values' => array () 
